...
public class UserController {
    ...
    public BaseResponse getUserByUsername(@PathVariable String username) {
        ...
        user.add(userService.findByUsername(username));
        return new SuccessResponse(0, user);
    }

    @GetMapping("/getReceipts/{username}")
    public BaseResponse getReceipts(@PathVariable String username) {
        List<Object> receipts = new ArrayList<>();
        receipts.add(userService.getReceipts(username));
